...
export default {
    ...
    methods: {
      async getStatistics(){
      this.$store.dispatch('GET_TRIPS_DATA',{
      ...
      updateDoughnutChart(){
      let iterator = this.$store.getters.STATS_TRIPS_DATA.map(data=>data.car).length;
       let colortab= [];
      ...
      let newChartData = {
      labels: this.$store.getters.STATS_TRIPS_DATA.map(data=>data.car),
      datasets: [
        {
          backgroundColor: colortab,
          data: this.$store.getters.STATS_TRIPS_DATA.map(data=>data.quantity)
        }
        ]
